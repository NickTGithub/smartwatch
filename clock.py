...
hr_tile = label.Label(font, text=display_hr, color=color, anchor_point = (0.5,0.5), scale=6)
hr_tile.x = 80
hr_tile.y = timer_y
...
apm_tile = label.Label(font, text=apm, color=color, anchor_point = (0.5,0.5), scale=2)
apm_tile.x = 180
...
def updateTime():
    global hours_today, current_time, last_time, mins, display_min, hr, display_hr, apm
...
day_tile = label.Label(font, text=display_day, color=color, anchor_point = (0.5,0.5), scale=date_scale)
day_tile.x = 70
day_tile.y = date_y

month_tile = label.Label(font, text=display_month, color=color, anchor_point = (0.5,0.5), scale=date_scale)
month_tile.x = 30
month_tile.y = date_y

slash = label.Label(font, text="/", color=color, anchor_point = (0.5,0.5), scale=date_scale)
slash.x = 55
slash.y = date_y

weekday_tile = label.Label(font, text=weekday, color=color, anchor_point = (0.5,0.5), scale=date_scale)
weekday_tile.x = 160
weekday_tile.y = date_y

date.append(day_tile)
date.append(month_tile)
date.append(slash)
date.append(weekday_tile)

def updateDate():
    global hours_today, day, display_day, weekday, weekday_index, weekday_list, \
        weekday_tile, day, day_threshold, day_threshold, day_threshold_index, \
        day_threshold_list, month, display_month, year
    if hours_today == 24:
        hours_today = 0
        day += 1
        weekday_index += 1
...
def updateTimeLabels():
    global display_min, display_hr, apm, display_day, display_month, weekday
